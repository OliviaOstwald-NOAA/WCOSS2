#!/bin/bash
set -x
export PS4=' + exevs_hurricane_regional_rmw_stats.sh line $LINENO: '
...
#  if [ ! -d ${comoutwpa} ]; then mkdir -p ${comoutwpa}; fi
#fi
export adeckfile=${COMINadeck}/a${stormBasin}${stormNumber}${stormYear}.dat
if [ -f ${adeckfile} ]; then
numrecs=`cat ${adeckfile} | wc -l`
...
if [ ${stormBasin} = "al" ]; then
  cp ${COMINadeckNHC}/a${stormBasin}${stormNumber}${stormYear}.dat ${STORMdata}/.
  ...
  cp ${COMINadeckJTWC}/a${stormBasin}${stormNumber}${stormYear}.dat ${STORMdata}/.
...
echo $(head -n 1 ${adeckfile}) > head.txt
echo $(tail -n 1 ${adeckfile}) > tail.txt
...
#--- run for TCRMW
#cp ${PARMevs}/metplus_config/${COMPONENT}/${STEP}/TCRMW_template.conf .
...
#  cp -r ${STORMroot}/tc_rmw/* ${comoutroot}/tc_rmw/.
  ...
  if [ ${stormBasin} = "al" ]; then
    cp ${comoutroot}/tc_stat/tc_stat.out ${comoutatl}/${stormBasin}${stormNumber}${stormYear}_rmw.out
  elif [ ${stormBasin} = "ep" ]; then
    cp ${comoutroot}/tc_stat/tc_stat.out ${comoutepa}/${stormBasin}${stormNumber}${stormYear}_rmw.out
  elif [ ${stormBasin} = "wp" ]; then
    cp ${comoutroot}/tc_stat/tc_stat.out ${comoutwpa}/${stormBasin}${stormNumber}${stormYear}_rmw.out
  fi
fi
